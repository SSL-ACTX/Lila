...
>(
    ...
    if !t_ctx.has_refinements {
        tracing::info!(target: "lirien::verify::verifier", "Skipping final consistency check for '{}' (no refinements).", func.name);
    } else {
        tracing::info!(target: "lirien::verify::verifier", "Performing final consistency check for '{}'...", func.name);
        if !t_ctx.backend.check()? {
    ...
    tracing::info!(target: "lirien::verify::verifier", "Proof successful for '{}' (inferred: {:?})", func.name, inferred);
    Ok(inferred)
}
    ...
    for block in &t_ctx.func.blocks {
        ...
        for inst in &block.instructions {
            match &inst.kind {
                ...
                | InstructionKind::ListCreate(..)
                | InstructionKind::ListAppend(..)
                | InstructionKind::ListLen(..)
                | InstructionKind::ListLoad(..)
                | InstructionKind::ListStore(..)
